...
fn uniform(name: &str, nums: &[f32]) -> (String, Uniform) {
	let unif: Uniform = match nums.len() {
		1 => Uniform::Float1(nums[0].to_string()),
		2 => Uniform::Float2(nums[0].to_string(), nums[1].to_string()),
		3 => Uniform::Float3(
			nums[0].to_string(),
			nums[1].to_string(),
			nums[2].to_string(),
...
async fn main() {
	let tex: Texture2D = Texture2D::from_rgba8(1, 2, &[0, 0, 32, 0, 64, 64, 64, 0]);
	...
	let mut uniforms: Vec<(String, Uniform)> = vec![uniform("TexSize", &[tex.width(), tex.height()])];

	let mut fragment_shader = ShaderBuilder::init("shaders/fs/")
		.add("head")
		.add("imports")
		.add("main")
		.build();
	...
	loop {
		...
		draw_plane(vec3(0., 0., 0.), vec2(1., 1.), tex, WHITE);
		gl_use_default_material();

		set_default_camera();

		let mut need_update = false;
		widgets::Window::new(hash!(), vec2(20., 20.), vec2(440., 550.))
			.label("Shader")
			.ui(&mut *root_ui(), |ui| {
				...
				ui.tree_node(hash!(), "Vertex shader", |ui| {
					if ui.editbox(hash!(), vec2(420., 200.), &mut vertex_shader) {
						need_update = true;
					};
				});
				TreeNode::new(hash!(), "Fragment shader")
					.init_unfolded()
					.ui(ui, |ui| {
						if ui.editbox(hash!(), vec2(420., 200.), &mut fragment_shader) {
							need_update = true;
						};
					});
				...
				if let Some(ref error) = error {
					...
					while let Some(line) = lines.next() {
						Label::new(line).multiline(14.0).ui(ui);
					}
				}
			});
		...
		if colorpicker_window {
			colorpicker_window &= widgets::Window::new(hash!(), vec2(140., 100.), vec2(210., 240.))
				...
				.ui(&mut *root_ui(), |ui| {
					...
					let color =
						color_picker_image.get_pixel(x.max(0).min(199) as u32, y.max(0).min(199) as u32);
...
void main() {
	gl_Position = 
		vec4(texcoord.x*2.-1., 1.-texcoord.y*2., 0., 1.);
	uv = texcoord;
}
";
